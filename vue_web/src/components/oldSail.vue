...
<script>
  ...
  export default {
    ...
    mounted() {
      ...
      let self = this;
      $.get("http://101.201.148.141:8083/goods/getRandomGoodsId.do",function (data) {
        self.submitGoodsForm.goodsId = data.goodsId;
        //设置上传图片路径，需要一个商品ID
        self.action = "http://101.201.148.141:8083/upload/uploadFile.do?goodsId=" + data.goodsId;
      },"json");
    },
    methods: {
      submitForm(formName) {
        if(this.submitFlag){
          this.$refs[formName].validate((valid) => {
            if (valid) {
              ...
              $.post("http://101.201.148.141:8083/goods/createGoods.do", jsonMsg, function (data) {
                if (data.code === 1) {
