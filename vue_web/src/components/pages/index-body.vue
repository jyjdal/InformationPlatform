...
<script>
    export default {
      ...
      mounted() {
        ...
        $.get("http://101.201.148.141:8083/goods/getIndexGoods.do/1",function (data) {
          self.data1 = data;
          $(self.data1).each(function (index,element) {
            let jsonObj = {};
            jsonObj.goodsId = element.goodsId;
            //为每个表格元素加载图片数据，主图
            $.get("http://101.201.148.141:8083/goods/getGoodsMainImg.do",jsonObj,function (data) {
              //本地映射到9090端口，部署到远程服务器需要修改这里，服务端返回的imgUrl应该为相对路径，这里图片名字就行
              element.picture = "http://101.201.148.141:9090/" + data.imgUrl;
        ...
        $.get("http://101.201.148.141:8083/goods/getIndexGoods.do/2",function (data) {
          self.data2 = data;
        ...
        $.get("http://101.201.148.141:8083/goods/getIndexGoods.do/3",function (data) {
          self.data3 = data;
      ...
      methods:{
        ...
        clickGoodsInfoButton(goodsId){
          ...
          $.get("http://101.201.148.141:8083/goods/getGoodsById.do",jsonObj,function (data) {
            self.goodsInfoName = data.name;
            self.goodsInfoPrice = data.price;
            self.goodsInfoDscrip = data.dscrip;
            $.get("http://101.201.148.141:8083/goods/getGoodsImgMap.do",jsonObj,function (data) {
        ...
        addToShopCar(){
          ...
          let jsonMsg = JSON.stringify(jsonObj);
          let self = this;
          $.post("http://101.201.148.141:8083/shopCar/addOneToShopCar.do",jsonMsg,function (data) {
            if(data.code === 1){
        ...
        clickSearch(){
          if(this.search !== "") {
            ...
            $.get("http://101.201.148.141:8083/goods/getSearchGoods.do", jsonObj, function (data) {
              //保存搜索内容
