...
<script>
    ...
    export default {
      ...
      mounted() {
        ...
        let self = this;
        //加载文字数据
        $.get("http://101.201.148.141:8083/shopCar/findAllByUid.do",jsonObj,function (data) {
          self.tableData = data;
          //加载商品主图
      ...
      methods: {
        ...
        handleEdit(index, row) {
          ...
          let jsonMsg = JSON.stringify(jsonObj);
          let self = this;
          $.post("http://101.201.148.141:8083/shopCar/deleteOneByUidAndGid.do", jsonMsg, function (data) {
            if (data.code === 1) {
        ...
        handleEdit2(index, row) {
          ...
          $.post("http://101.201.148.141:8083/shopCar/deleteOneByUidAndGid.do",jsonMsg,function (data) {
            ...
            if(data.code === 1){
              //创建订单
              $.post("http://101.201.148.141:8083/orders/createOrder.do",jsonMsg,function (data) {
                if(data.code === 1){
                  //模拟支付
                  //15秒后自动支付成功
                  setTimeout(function () {
        ...
        buyAll() {
          ...
          let jsonMsg = JSON.stringify(jsonObj);
          //创建订单
          $.post("http://101.201.148.141:8083/orders/createOrders.do",jsonMsg,function (data) {
            if(data.code === 1){
              //订单创建成功，删除购物车内商品
              $.post("http://101.201.148.141:8083/shopCar/deleteAllByUid.do",jsonMsg,function (data) {
